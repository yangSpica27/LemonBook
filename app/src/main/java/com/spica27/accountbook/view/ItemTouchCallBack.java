...
public class ItemTouchCallBack extends ItemTouchHelper.Callback {
  ...
  /**
   * 根据 RecyclerView 不同的布局管理器，设置不同的滑动、拖动方向 该方法使用 makeMovementFlags(int dragFlags, int swipeFlags)
   * 方法返回 参数: dragFlags:拖动的方向 swipeFlags:滑动的方向
   * @param recyclerView
   * @param viewHolder
   * @return
   */
  ...
  /**
   * 当 ItemTouchHelper 拖动一个Item时该方法将会被回调，Item将从旧的位置移动到新的位置 如果不拖动这个方法将从来不会调用,返回true表示已经被移动到新的位置
   *
   * @param recyclerView
   * @param viewHolder
   * @param target
  ...
  /**
   * 当Item被滑动的时候被调用 如果你不滑动这个方法将不会被调用
   *
   * @param viewHolder
   * @param direction
   */
